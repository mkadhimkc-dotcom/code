...
async function saveProfile(profile) {
  ...
  const { data, error } = await client.from('profiles').upsert(
    { username: profile.name, start_date: profile.startDate, auth_user_id: user.id },
    { onConflict: 'username' }
  ).select().single()
...
async function getWorkoutLogs(profileId) {
  const { data, error } = await client.from('workout_logs').select('*').eq('profile_id', profileId).order('created_at', { ascending: false })
...
async function saveCheckboxState(profileId, checkboxId, checked) {
  const { error } = await client.from('checkbox_states').upsert(
...
async function getCheckboxStates(profileId) {
  const { data, error } = await client.from('checkbox_states').select('checkbox_id, checked').eq('profile_id', profileId)
  if (error) return {}
  const map = {}
...
async function clearCheckboxStates(profileId) {
  const { error } = await client.from('checkbox_states').delete().eq('profile_id', profileId)
  if (error) console.error('clearCheckboxStates error:', error.message)
}
async function addCustomWorkout(assignment) {
  const { data, error } = await client.from('user_workouts').insert([assignment]).select().single()
  if (error) { console.error('addCustomWorkout error:', error.message); return null }
  return data
}
async function getCustomWorkouts(profileId) {
  const { data, error } = await client.from('user_workouts').select('*').eq('profile_id', profileId).order('sort_order', { ascending: true })
  if (error) { console.error('getCustomWorkouts error:', error.message); return [] }
  return data
}
async function removeCustomWorkout(assignmentId) {
  const { error } = await client.from('user_workouts').delete().eq('id', assignmentId)
  if (error) console.error('removeCustomWorkout error:', error.message)
}

window.supabaseHelper = {
  client, getSession, getUser, signOut, onAuthStateChange,
  saveProfile, loadProfile, loadProfileByAuthId,
  saveWorkoutLog, getWorkoutLogs,
  saveCheckboxState, getCheckboxStates, clearCheckboxStates,
  addCustomWorkout, getCustomWorkouts, removeCustomWorkout
}